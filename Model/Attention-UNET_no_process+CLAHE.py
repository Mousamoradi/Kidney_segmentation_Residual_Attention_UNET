...
att_unet_model.compile(optimizer=Adam(lr = 1e-3), loss=dice_coef_loss, 
...
csv_logger = CSVLogger(r'C:\Users\MOUSAMORADI\Desktop\newtry\log\attunet_no_process_{}.log'.format(datetime.now().strftime("%Y_%m_%d-%H_%M")))
callbacks = [ModelCheckpoint('attunet_no_proc1.hdf5', verbose=1, save_best_only=True), csv_logger]

reduce_lr = ReduceLROnPlateau(monitor='val_dice_coef', factor=0.2,patience=7, min_lr=1e-5,verbose=1)
start2 = datetime.now() 
att_unet_history =att_unet_model.fit(X_train, y_train, batch_size=BATCH_SIZE, epochs=EPOCHS, verbose=1, shuffle=True,
...
# future plotting
import pandas as pd    
att_unet_history_df1 = pd.DataFrame(att_unet_history.history) 
with open('att_unet_history_df1.csv', mode='w') as f:
    att_unet_history_df1.to_csv(f)
%matplotlib qt5
...
model_path = "attunet_no_proc1.hdf5"
...
img1 = cv2.imread(df_test['filename'].iloc[30])
...
mask = cv2.imread(df_test['mask'].iloc[30])
plt.subplot(1,4,2)
plt.imshow(mask)
